...
namespace BTCPayServer.Controllers
{
    ...
    public class RateController : Controller
    {
        ...
        public async Task<IActionResult> GetBaseCurrencyRates(string baseCurrency, string storeId, CancellationToken cancellationToken)
        {
            ...
            if (store == null)
            {
                var err = Json(new BitpayErrorsModel() { Error = "Tienda no encontrada" });
                err.StatusCode = 404;
                return err;
            }
        ...
        public async Task<IActionResult> GetRates2(string currencyPairs, string storeId, CancellationToken cancellationToken)
        {
            ...
            if (storeId == null)
            {
                var result = Json(new BitpayErrorsModel() { Error = "Debe especificar storeId (en la configuración de su tienda)" });
            ...
            if (store == null)
            {
                var result = Json(new BitpayErrorsModel() { Error = "Tienda no encontrada" });
            ...
            if (currencyPairs == null)
            {
                ...
                {
                    var result = Json(new BitpayErrorsModel() { Error = "Debe configurar los pares de divisas predeterminados en 'Configuraciones / tarifas de la tienda' o especificar el parámetro de consulta 'currencyPairs' (por ejemplo, BTC_USD, LTC_CAD)." });
                    result.StatusCode = 400;
                    return result;
                }
            }
            ...
            foreach (var currency in currencyPairs.Split(','))
            {
                ...
                {
                    var result = Json(new BitpayErrorsModel() { Error = $"Par de divisas {currency} formateado incorrectamente" });
                    result.StatusCode = 400;
                    return result;
                }
                pairs.Add(pair);
