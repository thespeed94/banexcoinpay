    ...
    public partial class WalletsController : Controller
    {
        ...
        private async Task<IActionResult> RedirectToWalletTransaction(WalletId walletId, Transaction transaction)
        {
            ...
            if (transaction != null)
            {
                ...
                StatusMessage = $"Transacción emitida con éxito. ({transaction.GetHash().ToString()})";
            }
            ...
            )
        {
            ...
            using (var signTimeout = new CancellationTokenSource())
            {
                ...
                try
                {
                    ...
                    if (command == "sendtoaddress")
                    {
                        if (!_dashboard.IsFullySynched(network.CryptoCode, out var summary))
                            throw new Exception($"{network.CryptoCode}: no iniciado o totalmente sincronizado");
                        ...
                        if (accountKey.AccountKeyPath == null)
                        {
                            ...
                            accountKey.AccountKeyPath = foundKeyPath ?? throw new HardwareWalletException($"Esta tienda no está configurada para usar este libro mayor.");
                        ...
                        else
                        {
                            ...
                            if (accountKey.RootFingerprint == null)
                            {
                                ...
                                if (!derivationSettings.AccountDerivation.GetExtPubKeys().Any(p => p.GetPublicKey() == actualPubKey.GetPublicKey()))
                                    throw new HardwareWalletException($"Esta tienda no está configurada para usar este libro mayor.");
                            }
                ...
                { result = new LedgerTestResult() { Success = false, Error = "Se acabó el tiempo" }; }
                catch (Exception ex)
            ...
            WalletId walletId, WalletSettingsViewModel vm)
        {
            ...
            StatusMessage = "Configuración de cartera actualizada";
