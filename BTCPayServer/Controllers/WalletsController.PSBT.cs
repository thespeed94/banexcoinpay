...
namespace BTCPayServer.Controllers
{
    public partial class WalletsController
    {
            ...
            WalletPSBTViewModel vm, string command = null)
        {
            ...
            if (psbt == null)
            {
                ModelState.AddModelError(nameof(vm.PSBT), "Inválido PSBT");
                return View(vm);
            }
            switch (command)
            {
            ...
            WalletId walletId, WalletPSBTReadyViewModel vm, string command = null)
        {
            ...
            if (command == "broadcast")
            {
                ...
                try
                {
                    ...
                    {
                        vm.GlobalError = $"RPC Error durante la transmisión: {broadcastResult.RPCCode} {broadcastResult.RPCCodeMessage} {broadcastResult.RPCMessage}";
                        return View(vm);
                    }
                }
                catch (Exception ex)
                {
                    vm.GlobalError = "Error durante la transmisión: " + ex.Message;
                    return View(vm);
                }
            ...
            WalletId walletId, WalletPSBTCombineViewModel vm)
        {
            ...
            if (sourcePSBT == null)
            {
                ModelState.AddModelError(nameof(vm.OtherPSBT), "Inválido PSBT");
                return View(vm);
            }
            sourcePSBT = sourcePSBT.Combine(psbt);
            StatusMessage = "PSBT combinado con éxito!";
