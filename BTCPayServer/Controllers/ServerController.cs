...
namespace BTCPayServer.Controllers
{
    ...
    public partial class ServerController : Controller
    {
        ...
        private IActionResult RunSSH(MaintenanceViewModel vm, string ssh)
        {
            ...
            if (_Options.TrustedFingerprints.Count != 0)
            {
                sshClient.HostKeyReceived += (object sender, Renci.SshNet.Common.HostKeyEventArgs e) =>
                {
                    if (_Options.TrustedFingerprints.Count == 0)
                    {
                        Logs.Configuration.LogWarning($"Huella dactilar de SSH para {e.HostKeyName} no es de confianza, inicie BTCPay con -sshtrustedfingerprints \"{Encoders.Hex.EncodeData(e.FingerPrint)}\"");
                    ...
                    else
                    {
                        ...
                        if (!e.CanTrust)
                            Logs.Configuration.LogError($"Huella dactilar de SSH para {e.HostKeyName} no es de confianza, inicie BTCPay con -sshtrustedfingerprints \"{Encoders.Hex.EncodeData(e.FingerPrint)}\"");
                    }
                };
            }
            else
            ...
            {
                ModelState.AddModelError(nameof(vm.Password), "Credenciales no válidas");
                sshClient.Dispose();
                return View(vm);
            }
            catch (Exception ex)
            {
                ...
                ModelState.AddModelError(nameof(vm.UserName), $"Problema de conexión ({message})");
                sshClient.Dispose();
                return View(vm);
            }
            ...
            sshCommand.BeginExecute(ar =>
            {
                try
                {
                    Logs.PayServer.LogInformation("Ejecutando el comando SSH: " + ssh);
                    var result = sshCommand.EndExecute(ar);
                    Logs.PayServer.LogInformation("Comando SSH ejecutado: " + result);
                }
                catch (Exception ex)
                {
                    Logs.PayServer.LogWarning("Error al ejecutar el comando SSH: " + ex.Message);
                }
                sshClient.Dispose();
            });
            return null;
        ...
        public new async Task<IActionResult> User(string userId, UserViewModel viewModel)
        {
            ...
            {
                viewModel.StatusMessage = "Este es el único administrador, por lo que su función no se puede eliminar hasta que se agregue otro administrador.";
            ...
            if (viewModel.IsAdmin != IsAdmin(roles))
            {
                ...
                viewModel.StatusMessage = "Usuario actualizado correctamente";
            }
        ...
        public async Task<IActionResult> DeleteUser(string userId)
        {
            ...
            if (IsAdmin(roles))
            {
                ...
                {
                    // return
                    return View("Confirm", new ConfirmModel("No se puede eliminar el último administrador",
                        "Este es el último administrador, por lo que no puede ser eliminado"));
                }

                return View("Confirm", new ConfirmModel("Eliminar administrador " + user.Email,
                    "¿Está seguro de que desea eliminar este administrador y eliminar todas las cuentas, usuarios y datos asociados con la cuenta del servidor??",
                    "Delete"));
            }
            else
            {
                return View("Confirm", new ConfirmModel("Borrar usuario " + user.Email,
        ...
        public async Task<IActionResult> Policies(PoliciesSettings settings)
        {
            ...
            TempData["StatusMessage"] = "Políticas actualizadas exitosamente";
        ...
        public async Task<IActionResult> Services()
        {
            ...
            {
                Name = storageSettings == null? "No establecido": storageSettings.Provider.ToString(),
        ...
        public async Task<IActionResult> Service(string serviceName, string cryptoCode, bool showQR = false, uint? nonce = null)
        {
            ...
            {
                StatusMessage = $"Error: {cryptoCode} no está completamente sincronizado";
            ...
            try
            {
                ...
                switch (service.Type)
                {
                    ...
                    case ExternalServiceTypes.Spark:
                        ...
                        {
                            StatusMessage = $"Error: La clave de acceso del servicio no está configurada.";
                            return RedirectToAction(nameof(Services));
                        }
        ...
        {
            await _SettingsRepository.UpdateSetting(settings);
            TempData["StatusMessage"] = "Ajustes de tema actualizados correctamente";
        ...
        public async Task<IActionResult> LogsView(string file = null, int offset = 0)
        {
            ...
            {
                vm.StatusMessage = "Error: Opción de registro de archivos no especificada. " +
                                   "Necesitas configurar debuglog y opcionalmente " +
                                   "debugloglevel en la configuración o mediante argumentos de tiempo de ejecución";
            }
            else
            {
                ...
                if (di == null)
                {
                    vm.StatusMessage = "Error: No se pudieron cargar los archivos de registro";
                }
