    ...
    public partial class ManageController : Controller
    {
        ...
        public async Task<IActionResult> Index(IndexViewModel model)
        {
            ...
            if (model.PhoneNumber != phoneNumber)
            {
                ...
                {
                    throw new ApplicationException($"Se produjo un error inesperado al configurar el número de teléfono para el usuario con ID'{user.Id}'.");
                }
            }
            ...
            if (needUpdate)
            {
                ...
                if (!result.Succeeded)
                {
                    throw new ApplicationException($"Se ha producido un error inesperado al actualizar el usuario con ID '{user.Id}'.");
                }
            }
        ...
        public async Task<IActionResult> LinkLoginCallback()
        {
            ...
            if (info == null)
            {
                throw new ApplicationException($"Se ha producido un error inesperado al cargar la información de inicio de sesión externa para el usuario con ID '{user.Id}'.");
            }

            var result = await _userManager.AddLoginAsync(user, info);
            if (!result.Succeeded)
            {
                throw new ApplicationException($"Se produjo un error inesperado al agregar un inicio de sesión externo para el usuario con ID '{user.Id}'.");
            }

            // Clear the existing external cookie to ensure a clean login process
            await HttpContext.SignOutAsync(IdentityConstants.ExternalScheme);

            StatusMessage = "Se ha añadido el inicio de sesión externo.";
