...
namespace BTCPayServer.Controllers
{
    public partial class AppsController
    {
        public class PointOfSaleSettings
        {
            ...
            public const string BUTTON_TEXT_DEF = "Comprar para {0}";
            public string ButtonText { get; set; } = BUTTON_TEXT_DEF;
            public const string CUSTOM_BUTTON_TEXT_DEF = "Pagar";
            public string CustomButtonText { get; set; } = CUSTOM_BUTTON_TEXT_DEF;
            public const string CUSTOM_TIP_TEXT_DEF = "¿Quieres dejar una consejo?";
        ...
        public async Task<IActionResult> UpdatePointOfSale(string appId)
        {
            ...
            if (HttpContext?.Request != null)
            {
                ...
                if (settings.ShowCustomAmount)
                {
                    ...
                    builder.AppendLine($"  <button type=\"submit\">Compra ahora</button>");
                ...
                try
                {
                    ...
                    builder.AppendLine($"  <input type=\"hidden\" name=\"notificationUrl\" value=\"https://example.com/callbacks\" />");
                    builder.AppendLine($"  <input type=\"hidden\" name=\"redirectUrl\" value=\"https://example.com/thanksyou\" />");
                    builder.AppendLine($"  <button type=\"submit\" name=\"choiceKey\" value=\"{items[0].Id}\">Compra ahora</button>");
                    builder.AppendLine($"</form>");
        ...
        public async Task<IActionResult> UpdatePointOfSale(string appId, UpdatePointOfSaleViewModel vm)
        {
            if (_currencies.GetCurrencyData(vm.Currency, false) == null)
                ModelState.AddModelError(nameof(vm.Currency), "Moneda invalida");
            try
            {
            ...
            {
                ModelState.AddModelError(nameof(vm.Template), "Plantilla inválida");
            }
